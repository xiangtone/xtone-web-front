...
var ad_library_18183 = newhanawa = {
    ...
    createAdBanner: function (name,ind) {
		var self = this;
		var ind=ind || 0;
		...
		var sw=nsName[0].seatwidth,
			sh=nsName[0].seatheight,
			ww=nsName[0].width,
			hh=nsName[0].height,
		...
		var s="";
		if(ind==1)s+="<div class=\"hr_10\"></div>";
		s+="<div class=\""+name+"_frame\" style=\"position:relative;z-index:1;margin:0 auto;overflow:hidden;width:" + sw + ";height:" + sh + ";\" ><div class=\"alladzou_frame\"  style=\"position:absolute;float:left;z-index:1;left:0;top:0;overflow:hidden;width:" + parseInt(bodyw)*parseInt(nsName.length)+"px;height:" + sh + ";\"  dt=\"dt_"+name+"\" pos=\"0\"  num=\""+nsName.length+"\"  w=\""+nsName[0].seatwidth.split("px")[0]+"\"  >";
		for(i in nsName){
			if(typeof(nsName[i]) == 'object'){
				s+="<style>@keyframes dt_"+name+i+"\
{\
to {left:-"+parseInt(nsName[i].seatwidth.split("px")[0])*i+"px;}\
}\
</style>";
				if(nsName[i].type == '5'){
	...
	createIndexAdBanner:function(name) {
		var self = this;
		self.createAdBanner(name,1);
    },
    ...
    float_ad_float: function (o, ww, hh) {
        ...
        o.css("right", 0);
        if (self.IE > 6) {
            o.css({"top": ((h - oh - 60)) + 'px', "position": "fixed"});
        } else {
            o.css({"top": (t + (h - oh - 60)) + 'px', "position": "absolute"});
        }
    },
    //------------------------------------广告展现------------------------------------
    //前弹
    ...
    bgfl_ad: function (name) {
        ...
        if (typeof(nsName[0])== 'object') {
            ...
            bgflAd.css({"display": "block", "height": "1024px", "position": "absolute", "z-index": "1px", "overflow": "hidden", "top": "45px", "left": "0", "width": "100%", "background": "url(" + nsName[0].src + ") center 0 no-repeat"});
			if ((this.ns['syqzttBanner'] instanceof Array)) {bgflAd.css({"top": "120px"});}
			if ((this.ns['ltqzttBanner'] instanceof Array)) {bgflAd.css({"top": "120px"});}
			if ((this.ns['qzttBanner'] instanceof Array)) {bgflAd.css({"top": "120px"});}
...
function topSlide_ad(id,nStayTime,sState,nMaxHth,nMinHth){ 
	...
	//console.log(this.state+"_"+w+"_"+(w/1680)*200);
	if(intervalId != null)window.clearInterval(intervalId); 
	function openBox(){ 
		var h = parseInt(obj.style.height.split("px")[0]);
		var sd=15; 
		//console.log(this.state+"_"+h);
		//console.log(this.state+"_"+h+"_"+this.maxHeigth);
		if(this.state == "down"){
		...
		jQuery(".top_toolbar").css("top",((this.state == "down") ? (h+ sd) : (h- sd))+"px");
