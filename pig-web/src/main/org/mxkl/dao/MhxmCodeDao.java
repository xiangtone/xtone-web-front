...
public class MhxmCodeDao {
	...
	public String checkOpenId(String openId){
		...
		try {
			...
			ps = con.prepareStatement("SELECT id FROM tbl_exchange_codes WHERE wechatOpenId = ? AND gameName='meng-huan-xian-mo'");
	...
	public String checkGameId(){
	    ...
	    try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("SELECT id FROM tbl_exchange_codes WHERE matchOpenIdTime IS NULL AND gameName='meng-huan-xian-mo' LIMIT 1");
			rs = ps.executeQuery();
	...
	public int updateOpenIdForId(String id,String openId,long time){
	    ...
	    try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("UPDATE tbl_exchange_codes SET wechatOpenId=?,matchOpenIdTime=? WHERE id=? AND gameName='meng-huan-xian-mo'");
			ps.setString(1, openId);
			ps.setLong(2, time);
			ps.setString(3, id);
