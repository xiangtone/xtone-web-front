  ...
  Lightbox = {
    getOptions: function() {
      return {
        ...
        Skin          : {
                          'black' : { 'hexcolor': '#ffffff', 'captionColor': '#d70f0f', 'background-color': '#000000', 'opacity': 0.5 }
                        }
      };
    },
    ...
    initialize: function(options) {
      ...
      var strBG = this.options.dir+'/'+((((window.XMLHttpRequest == undefined) && (ActiveXObject != undefined)))?this.options.backgroundIE:this.options.background);
      var name  = this.options.name;
      
      this.preloadimage(strBG);
      this.preloadimage(this.options.dir+'/'+this.options.buttons);
      ...
      this.lightbox = {};
			...
			$('body').append('<div id="'+name+'-Wrapper"><div id="'+name+'-Background"></div><div id="'+name+'-Contenedor"><div id="'+name+'-Top"><a id="'+name+'-CloseButton" title="关闭" href="#"></a><div id="'+name+'-Navegador"><strong id="'+name+'-Caption"></strong></div></div><div id="'+name+'-Contenido"></div><div id="'+name+'-Bottom"></div></div></div>');
...
/*      this.CloseButton.css({
        'background-image'  : 'url('+this.options.dir+'/'+this.options.buttons+')'
      });*/
    ...
    */
    ...
    getInfo: function (image, id) {
      ...
      IEuta.css({ 'background-image' : 'url('+this.options.dir+'/'+this.options.buttons+')' });
    ...
    show: function(caption, url, rel, force) {
      ...
      if ( force=='image' || baseURL.match(imageURL) )
      {
          ...
          this.img.onerror = $.bind(this, function() {
            this.show('', this.options.imagesdir+'/404.png', this.options.find);
          });
          ...
          else //AJAX
          {
            this.ResizeBox.bind('complete', $.bind(this, function(){
              $.ajax({
                ...
                type: "GET",
                cache: false,
                error: $.bind(this, function(){this.show('', this.options.imagesdir+'/404html.png', this.options.find)}),
    ...
    showNavBar: function() {
      if (this.navigator)
      {
        this.bb.addClass("lightbox-bbnav");
        this.Nav.empty();
        this.innerbb.empty();
      ...
      {
        this.bb.removeClass("lightbox-bbnav");
        this.innerbb.empty();
      }
    },
    ...
    showLoading: function() {
      ...
      this.Contenedor.css({
        'background' : 'url('+this.options.imagesdir+'/loader.gif) no-repeat 50% 50%'
      });
