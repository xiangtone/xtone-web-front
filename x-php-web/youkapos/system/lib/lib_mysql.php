...
final class Mysql {
	...
	private $db_user='huisen'; //数据库用户名
	private $db_pwd='dtsgx!@#'; //数据库用户名密码
	private $db_database='huisen'; //数据库名
	...
	//简化查询select
	public function selectRow($table, $columnName = "*", $condition = '') {
		$condition = $condition ? ' Where ' . $condition : NULL;
		$result = $this->query("SELECT $columnName FROM $table $condition");
		$row = mysql_fetch_assoc($result);
		return $row;
	}

	//简化查询select
	public function selectAll($table, $columnName = "*", $condition = '', $limit = "") {
		$condition = $condition ? ' Where ' . $condition : NULL;
	...
	public function update($table, $mod_content, $condition, $url = '') {
		$sql = "UPDATE $table SET $mod_content WHERE $condition";
		if ($this->query($sql)) {
			if (!empty ($url))
	...
	public function Get_admin_msg($url, $tips){
		echo '<script type="text/javascript">alert("'.$tips.'");location.href="'.$url.'";</script>';exit;
