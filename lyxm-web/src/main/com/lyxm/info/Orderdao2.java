...
public class Orderdao2 {
	...
	public Orderinfo selectById(long id){
		...
		try {
			...
			ps = con.prepareStatement("select * from tbl_orders_users_mz where id = ?");
			ps.setLong(1, id);
			rs = ps.executeQuery();
			if(rs.next()){
	...
	public int insert(Orderinfo order){
		...
		try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("insert into tbl_orders_users_mz values(?,?,?,?,?,?,?)");
	...
	public int updateGift(Orderinfo order){
		...
		try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("update tbl_orders_users_mz set gift=? where id = ?");
	...
	public int count(String phone){
		...
		try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("SELECT COUNT(*) as s FROM `tbl_orders_users_mz` WHERE invitepHOnenum = ?");
	...
	public Orderinfo login(long id,int codeNum){
		...
		try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("select * from tbl_orders_users_mz where id = ? and codeNum = ?");
			ps.setLong(1, id);
			ps.setInt(2, codeNum);
			rs = ps.executeQuery();
			if(rs.next()){
	...
	public int deleteUserById(String id){
		...
		try {
			con = ConnectionService.getInstance().getConnectionForLocal();
			ps = con.prepareStatement("DELETE FROM `tbl_orders_users_mz`  WHERE id = ?");
		    ps.setString(1, id);
