(function() {
	...
	var dcv={
		...
		//获取和设定session
		getChannel:function(){
			var adtag=dcv.getParameterByName("adtag").split(".");
			var session=dcv.getData("channel");			//从sessionStorage里获取渠道名称
			if(typeof(dcv.dcvDomain)!="undefined"&&dcv.dcvDomain.indexOf("baidu.com")>-1||dcv.dcvDomain.indexOf("sogou.com")>-1||dcv.dcvDomain.indexOf("haosou.com")>-1||dcv.dcvDomain.indexOf("hao76.com")>-1){//如果访前站点地址包含baidu.com||sogou.com||haosou.com||hao76.com
				dcv.channel="seo";
				session=dcv.setData("channel","seo");
			}
			else if(adtag.length>1){									//还需要加入一个判断,如果有adtag但是并不是dcv
				if(adtag[0]=="dcv"){
					dcv.channel=adtag[2];
					session=dcv.setData("channel",adtag[2]);
				}
				else if(adtag[0]=="media"||adtag[0]=="cop"){
					dcv.channel="media";
					session=dcv.setData("channel","media");
				}else{
					dcv.channel="other";
					session=dcv.setData("channel","other");
				}
			}
		...
		dcvReport:function(){
			dcv.getChannel();
			pgvSendClick({hottag:"dcv."+dcv.device+".pv."+dcv.channel});//每页都会上报的
			pgvSendClick({hottag:"dcv."+dcv.device+".uv."+dcv.channel});//每页都会上报的,到时候看dcv.device.uv的uv
	...
	dcv.action();
})()/*  |xGv00|d744f37aea6c645530f5897c93ccba6b */